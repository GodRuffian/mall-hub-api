...
class User extends Gapper\RObject
{
    ...
    protected function fetchRPC($criteria)
    {
        ...
        try {
            return (array) self::getRPC()->gapper->user->getInfo($user_id);
    ...
    public function getGroupInfo($group)
    {
        ...
        if (is_array($data)) {
            ...
            try {
                $data = self::getRPC()->gapper->user->getGroupInfo((int) $this->id, (int) $group->id);
            } catch (\Gini\RPC\Exception $e) {
            }
